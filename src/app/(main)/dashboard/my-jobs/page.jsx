		...
		<div className='flex flex-col gap-5 p-6'>
			...
			<div className='flex flex-col gap-2'>
				<div className='flex flex-wrap items-center gap-2'>
					...
					<Badge
						variant='secondary'
						className={`h-5 px-1.5 text-[11px] font-medium ${statusCfg.className}`}>
						{statusCfg.label}
					</Badge>
				</div>
				<div className='flex flex-wrap items-center gap-2 text-sm text-muted-foreground'>
					...
					{job.contract_type && (
						<Badge
							...
							{CONTRACT_LABEL[job.contract_type] ??
			...
			<Section title='Contrat & Planning'>
				<InfoGrid
					items={[
						job.work_time && [
							"Temps de travail",
							WORK_TIME_LABEL[job.work_time] ?? job.work_time,
						],
						job.work_schedule && [
							"Planning",
							WORK_SCHEDULE_LABEL[job.work_schedule] ??
								job.work_schedule,
						],
						job.weekly_hours && [
							"Heures / semaine",
			...
			<p className='text-xs text-muted-foreground'>
				...
				{job.updated_at &&
					...
					` · Modifiée le ${formatDate(job.updated_at)}`}
			</p>
		</div>
	);
}
...
export default function CompanyJobsPage() {
	...
	// Load current user + fetch their jobs
	useEffect(() => {
		async function init() {
			const {
				data: { user },
			...
			<div className='min-h-0 flex-1 overflow-hidden'>
				{loading ? (
					...
					<div className='flex flex-col items-center gap-3 rounded-xl border border-dashed py-16 text-center'>
						...
						<p className='text-sm text-muted-foreground'>
							Créez votre première offre d'emploi pour attirer des
							candidats.
						</p>
					...
					<div className='flex h-full gap-4'>
						...
						<div className='flex w-full flex-col gap-3 overflow-y-auto lg:w-96 xl:w-105 lg:shrink-0'>
			...
			<AlertDialog
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							L'offre <strong>"{deleteTarget?.title}"</strong>{" "}
