    ...
    <div className="flex items-center gap-2">
      {user.avatar && !imgError ? (
        ...
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border bg-muted">
          <span className="font-semibold text-muted-foreground text-xs">{initials || "?"}</span>
        </div>
      )}
      <span className="max-w-[140px] truncate font-medium text-sm">{user.name}</span>
    </div>
  );
}
...
export const transactionColumns = [
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="text-sm tabular-nums">{formatTransactionDate(row.original.created_at)}</span>,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (currency !== "EUR" || amount == null) return <span className="text-muted-foreground">—</span>;
      return <span className="font-medium tabular-nums">{Number(amount).toFixed(2)} €</span>;
    },
  },
  {
    accessorKey: "credits_added",
    header: ({ column }) => <DataTableColumnHeader column={column} title="Crédits ajoutés" />,
    cell: ({ row }) => {
      const credits = row.original.credits_added;
      if (!credits) return <span className="text-muted-foreground">—</span>;
      return <span className="font-medium text-blue-600 tabular-nums">+{credits}</span>;
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <Badge variant={currency === "EUR" ? "default" : "secondary"}>{currency ?? "—"}</Badge>;
    },
  },
];
