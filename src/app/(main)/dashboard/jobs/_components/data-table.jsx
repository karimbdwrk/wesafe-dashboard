...
import { Copy, Mars, Pencil, Plus, Venus, Zap } from "lucide-react";
...
const jobsColumns = [
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title="Sponsorisé jusqu'au" />,
    cell: ({ row }) => {
      ...
      return (
        <span
          className={`whitespace-nowrap font-medium text-sm ${isActive ? "text-amber-600" : "text-muted-foreground line-through"}`}
        >
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          <span className="max-w-[140px] truncate text-sm">{fullName}</span>
        </div>
      );
    },
    enableSorting: false,
  ...
  return (
    <>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="max-h-screen w-[400px] overflow-y-auto p-4">
          {!!selectedProfile && (
            <>
              <SheetHeader>
      ...
      <Dialog open={companyDialogOpen} onOpenChange={setCompanyDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Modifier le statut de l'entreprise</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4">
            <div className="flex justify-center gap-2">
              {["active", "pending", "rejected", "suspended"].map((status) => {
                const { color, label } = getCompanyStatusBadge(status);
                return (
                  <button
                    key={status}
                    type="button"
                  ...
                  >
                    <Badge variant={color} className="cursor-pointer px-4 py-2">
                      {label}
                    </Badge>
                  </button>
                );
