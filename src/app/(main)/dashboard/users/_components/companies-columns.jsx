...
function LogoCell({ logoUrl, name }) {
  ...
  if (!logoUrl) {
    return <span className="text-gray-400 text-xs">Aucun logo</span>;
  }
  return (
    <>
      <button type="button" className="focus:outline-none" onClick={() => setOpen(true)} title="Voir le logo en grand">
        <img
          src={logoUrl}
          alt={`${name} logo`}
          className="h-10 w-10 cursor-pointer rounded border object-cover transition-opacity hover:opacity-80"
        />
      </button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="flex max-w-lg items-center justify-center p-4">
          <DialogTitle>
            <VisuallyHidden>{name} – logo</VisuallyHidden>
          </DialogTitle>
          <img src={logoUrl} alt={`${name} logo`} className="max-h-[70vh] max-w-full rounded-lg object-contain" />
        </DialogContent>
      </Dialog>
    </>
  );
...
export const companiesColumns = [
  {
    ...
    header: "Logo",
    cell: ({ row }) => <LogoCell logoUrl={row.original.logo_url} name={row.original.name} />,
    size: 60,
    enableSorting: false,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <button
          ...
          className="focus:outline-none"
          onClick={() => {
            const event = new CustomEvent("openCompanyStatusDialog", { detail: row.original });
            window.dispatchEvent(event);
          }}
          title="Modifier le statut"
        >
          <Badge variant={color} className="inline-flex cursor-pointer items-center gap-2">
            <span className={`h-2 w-2 rounded-full ${dotColor}`} />
            {label}
          </Badge>
        </button>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant={color} className="inline-flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${dotColor}`} />
          {label}
        </Badge>
      );
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant="outline"
          className={`flex w-fit items-center gap-1 ${credits > 0 ? "border-amber-400 bg-orange-50 dark:bg-orange-950/30" : ""}`}
        >
          <span>⚡</span>
          {credits}
        </Badge>
