...
export const description = "Activité journalière des utilisateurs";
...
export function ChartAreaInteractive() {
  ...
  const [rawData, setRawData] = React.useState<Record<string, { events: number; users: number }>>({});
  ...
  React.useEffect(() => {
    supabase
      ...
      .then(({ data, error }) => {
        ...
        for (const [day, { events, userSet }] of Object.entries(byDay)) {
          normalized[day] = { events, users: userSet.size };
        }
  ...
  // Génère tous les jours de la période, avec 0 si pas de data
  const filteredData = React.useMemo(() => {
    const daysToSubtract = timeRange === "7d" ? 7 : timeRange === "30d" ? 30 : 90;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const days: { date: string; events: number; users: number }[] = [];
    for (let i = daysToSubtract - 1; i >= 0; i--) {
      const d = new Date(today);
      d.setDate(d.getDate() - i);
      const key = d.toISOString().slice(0, 10);
      days.push({ date: key, events: rawData[key]?.events ?? 0, users: rawData[key]?.users ?? 0 });
    }
    return days;
  }, [timeRange, rawData]);
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        ...
        <CardDescription>
          <span className="@[540px]/card:block hidden">Événements et utilisateurs uniques par jour</span>
          <span className="@[540px]/card:hidden">Activité journalière</span>
        </CardDescription>
        <CardAction>
          <ToggleGroup
            type="single"
          ...
          <AreaChart data={filteredData}>
            <defs>
              <linearGradient id="fillEvents" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="var(--color-events)" stopOpacity={1.0} />
