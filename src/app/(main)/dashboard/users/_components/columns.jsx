...
import { Calendar, ChevronRight, ExternalLink, FileText, Hash, Pencil, UserCheck } from "lucide-react";
...
async function sendProfileStatusNotification({ profile, newStatus, rejectReason }) {
  const {
    data: { user },
  ...
  if (newStatus !== "pending" && profile.email) {
    try {
      ...
      console.error("[sendProfileStatusNotification] Erreur envoi email:", err);
    }
  }
}
...
async function sendDocumentStatusNotification({ doc, tableName, newStatus }) {
  ...
  try {
    ...
    console.error("[sendDocumentStatusNotification] Erreur envoi email:", err);
  }
}
    ...
    <>
      <button type="button" className="focus:outline-none" onClick={() => setOpen(true)}>
        <img
          src={url}
          alt="avatar"
          className="h-8 w-8 shrink-0 cursor-pointer rounded-full object-cover transition-opacity hover:opacity-80"
          onError={() => setError(true)}
        />
      </button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="flex max-w-sm items-center justify-center p-4">
          <DialogTitle>
            <VisuallyHidden>
              {firstname} {lastname} – avatar
            </VisuallyHidden>
          </DialogTitle>
          <img src={url} alt="avatar" className="max-h-[70vh] max-w-full rounded-full object-contain" />
        </DialogContent>
      </Dialog>
    </>
  );
...
function SocialSecurityModal({ row }) {
  ...
  const [status, setStatus] = useState(row.original.social_security_verification_status ?? "pending");
  ...
  const handleChange = async () => {
    ...
    if (error) {
      ...
      if (user && row.original.id) {
        const ssDocLabel = docTypeLabel[row.original.social_security_document_type] || "document Sécurité Sociale";
        const statusMsg =
          {
            verified: "validé",
            rejected: "refusé",
          ...
          try {
            await supabase.functions.invoke("send-document-status-email", {
              body: {
                ...
                recipientName: row.original.first_name || row.original.email,
                documentLabel: ssDocLabel,
                status,
              },
            });
          } catch (err) {
            console.error("[SocialSecurityModal] Erreur envoi email:", err);
          }
        }
      }
    }
    ...
    <>
      ...
      >
        <DialogTrigger asChild>
          <span className="cursor-pointer">{getStatusBadge(row.original.social_security_verification_status)}</span>
        </DialogTrigger>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="flex flex-col items-center gap-3 pt-2 text-center">
            ...
            <div className="space-y-1">
              <DialogTitle className="font-semibold text-base">Vérification Sécurité Sociale</DialogTitle>
              <p className="text-muted-foreground text-sm">
                {docTypeLabel[row.original.social_security_doc_type] || "Document non renseigné"}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {signedUrl && (
              <img
                ...
                className="max-h-40 w-full cursor-pointer rounded-lg border object-cover"
            ...
            <div className="space-y-1.5">
              <label className="font-medium text-sm">Numéro de sécurité sociale</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              ...
              <Select value={status} onValueChange={setStatus}>
                ...
                <SelectContent>
                  {Object.entries(docVerifStatusConfig).map(([value, cfg]) => (
            ...
            <Button className="w-full" disabled={saving} onClick={handleChange}>
...
function StatusModal({ row, updateRowStatus }) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState(row.original.profile_status ?? "pending");
  ...
  const handleChange = async () => {
    ...
    updatePayload.reject_message = selected === "rejected" ? rejectReason.trim() : null;
    ...
    if (error) {
      ...
      await sendProfileStatusNotification({
        ...
        newStatus: selected,
        rejectReason: selected === "rejected" ? rejectReason.trim() : undefined,
      });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <span className="cursor-pointer">{getProfileStatusBadge(row.original.profile_status)}</span>
      </DialogTrigger>
      <DialogContent className="sm:max-w-sm">
        {/* Icône */}
        <div className="flex flex-col items-center gap-3 pt-2 text-center">
          ...
          <div className="space-y-1">
            <DialogTitle className="font-semibold text-base">Statut du profil</DialogTitle>
        ...
        {/* Select */}
        <div className="space-y-1.5">
          <label className="font-medium text-sm">Nouveau statut</label>
        ...
        <Button
          ...
          disabled={saving || (selected === "rejected" && !rejectReason.trim())}
      ...
      if (user && row.original.id) {
        ...
        if (row.original.email) {
          try {
            ...
            console.error("[IdVerificationModal] Erreur envoi email:", err);
          }
        }
      }
    }
      ...
      >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col items-center gap-3 pt-2 text-center">
            ...
            <div className="space-y-1">
              <DialogTitle className="font-semibold text-base">Vérification d'identité</DialogTitle>
              <p className="text-muted-foreground text-sm">{idTypeLabel[idType] || "Document non renseigné"}</p>
            </div>
          </div>
          ...
          <div className="mt-2 flex flex-col gap-4">
            ...
            {["residence_permit"].includes(idType) && (
              <div>
                ...
                />
                {loadingNationalities && <div className="text-muted-foreground text-xs">Recherche...</div>}
                {nationalitySuggestions.length > 0 && (
                  <ul className="mt-1 max-h-40 overflow-auto rounded border bg-white dark:bg-zinc-900">
                    {nationalitySuggestions.map((c) => (
                      <li
                        key={c.code}
                        className="flex cursor-pointer items-center gap-2 px-2 py-1 hover:bg-blue-100 dark:hover:bg-blue-900/40"
                ...
                {selectedNationality && (
                  <div className="mt-2 text-green-700 text-sm dark:text-green-400">
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-5">
            {/* Aperçu du document */}
            {doc.document_url && (
              <div className="flex min-h-30 items-center justify-center rounded-lg border bg-muted/40 p-3">
                {loadingUrl ? (
...
function DocCard({ doc, onClick }) {
  ...
  return (
    <button type="button" className="group w-full text-left focus:outline-none" onClick={onClick}>
      <Card className="cursor-pointer border transition-all duration-150 hover:border-blue-400 hover:shadow-md group-focus-visible:ring-2 group-focus-visible:ring-blue-400 dark:hover:border-blue-500">
        <CardContent className="p-4">
          <div className="flex items-start justify-between gap-3">
            {/* Icône + titre */}
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-950/50">
                <FileText className="h-5 w-5 text-blue-500" />
              </div>
            ...
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t pt-3">
              {doc.expires_at &&
                (() => {
                  ...
                  const isExpired = expDate < new Date();
                  return isExpired ? (
                    <span className="flex items-center gap-1 font-medium text-red-600 text-xs dark:text-red-400">
    ...
    <span className="ml-1 flex items-center gap-1">
      {summary.verified > 0 && (
        <span className="flex items-center gap-0.5">
          ...
          <span className="font-medium text-green-700 text-xs dark:text-green-400">{summary.verified}</span>
        </span>
      )}
      {summary.pending > 0 && (
        <span className="flex items-center gap-0.5">
          <span className="h-1.5 w-1.5 rounded-full bg-yellow-400" />
          <span className="font-medium text-xs text-yellow-600 dark:text-yellow-400">{summary.pending}</span>
        </span>
      )}
      {summary.rejected > 0 && (
        <span className="flex items-center gap-0.5">
          <span className="h-1.5 w-1.5 rounded-full bg-red-500" />
          <span className="font-medium text-red-600 text-xs dark:text-red-400">{summary.rejected}</span>
        </span>
      )}
    </span>
  );
  ...
  return (
    <>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="max-h-screen w-120 overflow-y-auto p-5">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return !row.original.id_type ? (
        <Badge variant="outline" className="inline-flex cursor-default items-center gap-2 text-muted-foreground">
